...
module.exports = function computeBudget(userid, budget, transactions) {
  ...
  switch (newBudget.type) {
    ...
    case "quarterly":
      ...
      perdiods.current.start = new Date(
        currentYear,
        (currentQuarter - 1) * 3,
        1
      );
      ...
      if (currentQuarter === 1) {
        ...
        perdiods.previous.start = new Date(
          currentYear,
          (currentQuarter - 2) * 3,
          1
        );
    ...
    case "sliding":
      ...
      perdiods.current.start =
        nowDate - budget.slidingDuration * (1000 * 3600 * 24);
      perdiods.current.end = nowDate;
      perdiods.current.period = "slidingCurrent";
      perdiods.previous.target = budget.targets[0].target;
      perdiods.previous.end = perdiods.current.start;
      perdiods.previous.start =
        nowDate - 2 * budget.slidingDuration * (1000 * 3600 * 24);
      perdiods.previous.period = "slidingPrevious";
      break;
    default:
      return newBudget;
  }
  perdiods.current.progress =
    (nowDate - perdiods.current.start) /
